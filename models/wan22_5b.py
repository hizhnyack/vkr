# -*- coding: utf-8 -*-
"""Wan 2.2 TI2V-5B: полный GPU при VRAM >= 24 ГБ, иначе CPU offload."""
...
def run(
    ...
    try:
        ...
        if use_full_gpu(_FULL_GPU_VRAM_GB, "Wan 2.2 5B", log):
            pipe.to("cuda")
        else:
        ...
        for i, prompt in enumerate(prompts):
            log("Генерация сегмента %d/%d (Wan 2.2 5B)..." % (i + 1, len(prompts)))
            out = pipe(
                prompt=prompt,
                num_frames=81,
                height=704,
        ...
        log("Wan 2.2: модуль не установлен (%s), используем заглушку." % e)
        return run_stub(prompts, output_dir, log)
    except Exception as e:
        log("Ошибка Wan 2.2: %s" % e)
        raise RuntimeError("Wan 2.2: %s" % e)
