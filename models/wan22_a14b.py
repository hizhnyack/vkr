# -*- coding: utf-8 -*-
"""Wan 2.2 T2V-A14B: полный GPU при VRAM >= 70 ГБ, иначе CPU offload."""
...
_FULL_GPU_VRAM_GB = 70

metadata = {
    "id": "wan22_a14b",
    "name": "Wan 2.2 A14B",
    "description": "720p, ~70 GB VRAM, без offload",
    "min_vram_gb": 70,
}
...
def run(
    ...
    try:
        ...
        if use_full_gpu(_FULL_GPU_VRAM_GB, "Wan 2.2 A14B", log):
            pipe.to("cuda")
        else:
        ...
        for i, prompt in enumerate(prompts):
            log("Генерация сегмента %d/%d (Wan 2.2 A14B)..." % (i + 1, len(prompts)))
            out = pipe(
                prompt=prompt,
                num_frames=81,
                height=704,
        ...
        log("Wan 2.2 A14B: модуль не установлен (%s), используем заглушку." % e)
        return run_stub(prompts, output_dir, log)
    except Exception as e:
        log("Ошибка Wan 2.2 A14B: %s" % e)
        raise RuntimeError("Wan 2.2 A14B: %s" % e)
